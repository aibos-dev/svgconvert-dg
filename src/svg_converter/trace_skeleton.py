...
def process_and_save_image(input_image_path, output_image_path):
    ...
    im = (im0[:, :, 0] < 180).astype(np.uint8)

    # Perform skeletonization
    im = thinning(im)

    # Initialize an empty list for rectangles
    rects = []
    polys = traceSkeleton(im, 0, 0, im.shape[1], im.shape[0], 10, 999, rects)

    # Draw skeleton traces on the image
    for l in polys:
...
if __name__ == "__main__":
  ...
  input_folder = './skeletonization_test_data_clean'  # Folder with images to process
  output_folder = './processed_images_clean'  # Foldger to save processed images
